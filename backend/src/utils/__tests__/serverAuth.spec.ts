...
describe("buildServerAuth", () => {
    it("should return API key-based ServerAuth when no OAuth config is provided", async () => {
        ...
        const serverAuth = buildServerAuth(server);
...
describe("OAuthServerAuth", () => {
    it("should obtain and cache an access token", async () => {
        ...
        const datastore = provideDatastore(`oauth-${server.id}`, {expireAfterSeconds: 10 * 60});
        const cachedToken = await datastore.connect("tokens", async (col) => {
            return col.findOne({
                serverId: server.id,
                tokenType: 0, // StoredTokenType.ACCESS_TOKEN
            });
        });
        expect(cachedToken).not.toBeNull();
        expect(cachedToken?.encryptedToken).toBe("encrypted-test-access-token");
        expect(cachedToken?.tokenExpiry).toBeGreaterThan(Date.now());
        expect(cachedToken).not.toBeNull();
        expect(cachedToken?.encryptedToken).toBe("encrypted-test-access-token");
    });
});
