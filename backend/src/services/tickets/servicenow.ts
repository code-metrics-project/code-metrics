...
class ServiceNowClient {
  ...
  constructor(server: ServiceNowServer) {
    this.server = server;
    this.auth = buildServerAuth(server);
  }
  ...
  async listTickets(query: ServiceNowQuery): Promise<ServiceNowTicket[]> {
    ...
    do {
      page++;
      const url = `${this.server.url}/api/now/table/${query.tableName}`
        + `?sysparm_limit=${limit}`
      ...
      const response: ServiceNowResponse = await limitConcurrency(limiter, async () => fetch(url, options)
        .then((res) => res.json())
  ...
  private composeQuery(query: ServiceNowQuery): string {
    let composedQuery = query.query
      .map(({ key, value, operator }) => `${key}${operator}${value}`)
      .join("^");
...
export class ServiceNowTicketService implements TicketService {
  ...
  fetchTickets = async (
    ...
    const additionalQuery = ticketManagement?.teamFilterQuery
      ? `^${ticketManagement.teamFilterQuery}`
      : undefined;
