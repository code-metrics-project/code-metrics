...
export const initNoOpPipelines = () => registerPipelines(PipelinesTypes.NONE, (stage) => new NoOpPipelineService(stage));

class NoOpPipelineService extends AbstractPipelinesService {
    /* eslint-disable */

    discoverJobNames = async (
        workload: Workload,
        jobGroup: string
    ): Promise<string[]> => {
        return  [];
    ...
    getRunById = (workloadId: WorkloadId, jobName: string, runId: string): Promise<RunWithMetadata | null> => Promise.resolve(null);
}
