...
describe('Lead Time for Changes', () => {
  jest.mock("../../deployment/deploymentService", () => ({
    getDeploymentService: jest.fn().mockReturnValue({
      calculateLeadTimes: jest.fn().mockResolvedValue(new Map([
        [new Date("2011-04-19"), new Map([
          ["backend", {
            count: 1,
  ...
  it('should calculate lead time for changes', async () => {
    ...
    expect(firstDay["lead-time"].filter((l) => l.dimensions.workloadId === "athena" && l.dimensions.jobGroup === "backend")).toStrictEqual([{
    ...
    expect(secondDay["lead-time"].filter((l) => l.dimensions.workloadId === "athena" && l.dimensions.jobGroup === "backend")).toStrictEqual([{
