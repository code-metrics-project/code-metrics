...
export const fetchNonWorkingPatternChanges = async (
  args: NonWorkingPatternArgs,
): Promise<Map<DateStamp, DatedMetrics>> => {
  logger(
    `Fetching non-working pattern changes for workloads: ${args.workloads} and repo groups: ${args.repoGroups} from: ${args.startDate}`,
  );
  try {
    ...
    throw new Error(
      `Failed to fetch non-working pattern changes for workloads: ${args.workloads} and repo groups: ${args.repoGroups}: ${error}`,
    );
  }
};
