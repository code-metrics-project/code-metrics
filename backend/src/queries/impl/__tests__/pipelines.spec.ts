...
describe('fetchPipelineRuns', () => {
  ...
  it('should Call getPipelineRunsWithArgs correctly', async () => {
    ...
    const runs = await pipelines.fetchPipelineRuns({
      workloads, startDate, jobGroups, branchNames, valueFormat, actorType, stageId,
    });
    ...
    const successful = runsSuccessful.filter((metric) => metric.dimensions.workloadId === "workload1" && metric.dimensions.repoName === "octo-repo");
    expect(successful).toHaveLength(1);
    expect(successful[0].date.toISOString()).toEqual("2011-04-19T00:00:00.000Z");
    expect(successful[0].value).toEqual(1);

    const failed = runsFailed.filter((metric) => metric.dimensions.workloadId === "workload1" && metric.dimensions.repoName === "octo-repo");
    expect(failed).toHaveLength(1);
    expect(failed[0].date.toISOString()).toEqual("2011-04-19T00:00:00.000Z");
    expect(failed[0].value).toEqual(0);

    const aborted = runsAborted.filter((metric) => metric.dimensions.workloadId === "workload1" && metric.dimensions.repoName === "octo-repo");
  ...
  it('should handle errors when getPipelineRunsWithArgs throws', async () => {
    ...
    await expect(pipelines.fetchPipelineRuns({
      workloads, startDate, jobGroups, branchNames, valueFormat, actorType, stageId,
    }))
