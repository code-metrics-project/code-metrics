...
const { owner, repo, commit_sha } = req.pathParams;
...
// Usually a commit is part of 1 PR, sometimes 0, rarely 2
const prCount = 1; // Ideally we would simulate more variety in this mock, but it makes testing flaky until we refactor the mock behaviour significantly // Math.random() < 0.1 ? 0 : Math.random() < 0.9 ? 1 : 2;
const prs = [];
const now = new Date();
...
for (let i = 0; i < prCount; i++) {
  ...
  prs.push({
    ...
    statuses_url: `https://api.github.com/repos/${owner}/${repo}/statuses/${commit_sha}`,
    number: prNumber,
    state: "closed",
    locked: false,
    ...
    _links: {
      ...
      statuses: {
        href: `https://api.github.com/repos/${owner}/${repo}/statuses/${commit_sha}`,
      },
    },
...
console.debug(`Generated ${prs.length} PRs for commit ${commit_sha} in ${owner}/${repo}`);
