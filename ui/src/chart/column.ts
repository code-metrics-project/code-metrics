...
export function createColumnChartDatasets(inputs: Map<string, DatedMetrics>[]): ColumnChartData {
  ...
  inputs.forEach((input) => {
    groupByName(input).forEach((data: DatedValue[], name) => {
      ...
      columns.data.push(...data.map((d) => ({ x: d.date.getTime(), y: round(d.value, 1) })));
      series.push(columns);
