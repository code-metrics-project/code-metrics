    ...
    <v-row>
      ...
      >
        <v-card class="flex-grow-1 d-flex" style="flex-direction: column">
          <v-card-title
            ><v-icon class="mr-2" :color="workload.color">mdi-circle</v-icon
            >{{ workload.name }}</v-card-title
          >
          <v-card-text class="flex-grow-1">
            <v-list>
              <v-list-item
                v-for="(count, repo) in workload.repos"
                :key="repo"
                class="workload-repo py-0"
              >
          ...
          <v-card-actions>
            <v-btn
              :to="`${Paths.Workloads}/${workload.id}`"
              color="primary"
              plain
              >View workload</v-btn
            >
