<template>
  <v-card class="mb-2">
    <v-sheet color="grey-lighten-4" class="pt-3" v-for="summary in summaries" :key="summary.title">
      <v-card-subtitle class="subtitle-1 font-weight-bold">{{ summary.title }}</v-card-subtitle>
...
export default {
  ...
  mounted() {
    ...
    const filtered = (this.metrics as unknown as QueryAndResult[]).filter((qr) =>
      this.summarise.includes(qr.queryName),
    );
