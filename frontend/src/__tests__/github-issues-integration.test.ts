...
describe("GitHub Issues Integration Tests", () => {
  ...
  describe("Data Processing and Transformation", () => {
    ...
    it("should handle time series data for GitHub issues correctly", () => {
      type GithubIssue = (typeof mockRawGithubIssues)[number];
  ...
  describe("Error Handling", () => {
    it("should handle API errors gracefully for GitHub bugs query", async () => {
      ...
      try {
        ...
        expect(typedError.response.data.message).toBe("Internal server error");
      }
    });
    ...
    it("should handle API errors gracefully for GitHub incidents query", async () => {
      ...
      try {
        ...
        expect((error as { response: { status: number; data: { message: string } } }).response.status).toBe(404);
        expect((error as { response: { status: number; data: { message: string } } }).response.data.message).toBe(
          "Workload not found"
        );
      }
    });
